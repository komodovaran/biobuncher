...
def single_lstm_autoencoder(n_timesteps, n_features, latent_dim):
...
def multi_lstm_autoencoder(n_timesteps, n_features, latent_dim):
    ...
    x = TimeDistributed(Dense(n_features, activation = None))(x)
...
def this_works_lstm(n_timesteps, n_features, latent_dim):
    """Works with standardized inputs"""
    input = Input(shape=(n_timesteps, n_features))
