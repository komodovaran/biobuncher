...
def lstm_autoencoder(
    n_features, n_timesteps, latent_dim, bidirectional
):
    """
    Parameters
    ----------
    ...
    """
    # def _repeat(x):
    #     step_matrix = K.ones_like(x[0][:, :, :1])  # matrix with ones, shaped as (batch, steps, 1)
    #     latent_matrix = K.expand_dims(x[1], axis = 1)  # latent vars, shaped as (batch, 1, latent_dim)
    #     return K.batch_dot(step_matrix, latent_matrix)

    # ENCODER
    p = dict(units = latent_dim, return_sequences = False)

    inputs = Input(shape = (None, n_features))
    if bidirectional:
        ez = Bidirectional(CuDNNLSTM(**p), merge_mode = "sum")(inputs)
    else:
        ez = CuDNNLSTM(**p)(inputs)

    encoder = Model(inputs = inputs, outputs = ez)

    # DECODER
    p["return_sequences"] = True
    latent = Input(shape = (latent_dim,))
    dz = RepeatVector(n_timesteps)(latent)  # repeat latent_dim n_timesteps times
    if bidirectional:
        dz = Bidirectional(CuDNNLSTM(**p), merge_mode = "sum")(dz)
    else:
        dz = CuDNNLSTM(**p)(dz)

    dz = Activation(None)(dz)
    outputs = TimeDistributed(Dense(n_features))(dz)
...
def variable_lstm_autoencoder(latent_dim, n_features):
    ...
    encoded = Bidirectional(CuDNNLSTM(latent_dim, name="encoded"), merge_mode = "sum")(inputs)
    decoded = Lambda(variable_repeat)([inputs, encoded])
    outputs = CuDNNLSTM(n_features, return_sequences=True)(decoded)
    autoencoder = Model(inputs=inputs, outputs=outputs)
    autoencoder.compile(optimizer="adam", loss="mse")
    return autoencoder


def conv_autoencoder(n_features, n_timesteps, latent_dim, activation = "relu"):
    """
    Parameters
    ----------
    n_features:
...
def dilated_conv_autoencoder(n_features, n_timesteps, latent_dim):
    """
    Parameters
    ----------
    n_features:
...
def residual_conv_autoencoder(
    n_features, n_timesteps, latent_dim, activation = "relu", optimizer = "adam"
):
    """
