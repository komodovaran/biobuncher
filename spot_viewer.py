...
def _plot_trace(time, values, colors):
    fig, ax = plt.subplots()
    for i, val in enumerate(values):
        ax.plot(time, val, color = colors[i])
    ax.axvline(frame_idx, color = "black", ls = "--")
    ax.set_xlim(real_time.min(), real_time.max())
    if len(real_time) < 15:
        ax.set_xticks(np.arange(min(real_time), max(real_time), 3))
    ax.set_xlabel("Frame")
...
if __name__ == "__main__":
    ...
    TOP_VIDEO_DIR = "/media/tklab/linux-data/Data"
    ...
    frame_idx = st.sidebar.number_input(
        min_value = 1, max_value = video.shape[0], label = "Video frame", step = 1
    )
    first_frame = real_time[0]
    last_frame = real_time[-1]
    corrected_idx = int(frame_idx - first_frame)

    # Fix index out of particle range
    if corrected_idx < 0:
        corrected_idx = 0
    elif corrected_idx >= len(real_time):
        corrected_idx = len(real_time) - 1
    else:
        pass

    st.write("**CME file:**", cme_file_path)
    st.write("**Video file:**", video_file_path)
    st.write("**First frame**: {:d}".format(int(first_frame)),
             "**Last frame**: {:d}".format(int(last_frame)),
             "**Movie length**: {:d}".format(int(tmax)))

    linemap = lib.plotting.get_colors("plasma", n_colors = 2)
    _plot_trace(time = real_time, values = [int_c0, int_c1], colors = linemap)
    ...
    x0, x1, y0, y1 = _zoomed_pos(x = x_mean, y = y_mean, pad = 30)

    for c in [0, 1]:
        # normal axes
        axes[c].set_title("Channel {}".format(c))
        axes[c].imshow(video[frame_idx, ..., c], cmap = "inferno_r")
        axes[c].text(**textp)
        axes[c].set_xticks(())
        axes[c].set_yticks(())
        ...
        axes[c + 2].imshow(video[frame_idx, ..., 0], cmap = "inferno_r")
        axes[c + 2].set_xlim(x0, x1)
        axes[c + 2].set_ylim(y0, y1)

    x_ = int(x_coords[corrected_idx])
    y_ = int(y_coords[corrected_idx])
